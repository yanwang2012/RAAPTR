...
void ptapso_debug(size_t nDim, 
            double (*fitfunc)(const gsl_vector *, void *), void *ffParams, 
	...
	*/
	...
	double rngNum;
	FILE *rngFile = psoParams.rngFile;
	if (rngFile != NULL)
		printf("Warning: Random numbers will be read from file!\n");
	if (psoParams.debugDumpFile != NULL)
		printf("Output will be dumped in a file\n");
	...
	func2minimz.f = fitfunc; /* Name of function to minimize */
	func2minimz.params = ffParams; /* Parameters needed by this function */
	...
	*/
	...
	for (lpParticles = 0; lpParticles < popsize; lpParticles++){
	      initPsoParticles_debug(&pop[lpParticles], nDim, rngGen, rngFile);
    }	
	...
	*/
	for (lpPsoIter = 1; lpPsoIter <= maxSteps-1; lpPsoIter++){
		...
		for (lpParticles = 0; lpParticles < popsize; lpParticles++){
	        ...
	        pop[lpParticles].partFitEvals = pop[lpParticles].partFitEvals+funcCount;
		...
		//particleInfoDump(stdout,pop,popsize);
	    ...
	    for (lpParticles = 0; lpParticles < popsize; lpParticles++){
			...
			for (lpCoord = 0; lpCoord < nDim; lpCoord++){
				if (rngFile == NULL)
					...
					fscanf(rngFile,"%lf",&rngNum);
				//printf("%f\n",rngNum);
				gsl_vector_set(chi1Vec,lpCoord,rngNum);
			}
			for (lpCoord = 0; lpCoord < nDim; lpCoord++){
				if (rngFile == NULL)
					rngNum = gsl_rng_uniform(rngGen);
					...
					fscanf(rngFile,"%lf",&rngNum);
				//printf("%f\n",rngNum);
				gsl_vector_set(chi2Vec,lpCoord,rngNum);
			}
...
void initPsoParticles_debug(struct particleInfo *p, size_t nDim, gsl_rng *rngGen, FILE *rngFile){
	...
	p->partCoord = gsl_vector_calloc(nDim);
	for (lpCoord = 0; lpCoord < nDim; lpCoord++){
		if (rngFile == NULL)
			rngNum = gsl_rng_uniform(rngGen);
		else
	...
	p->partVel = gsl_vector_calloc(nDim);
	for (lpCoord = 0; lpCoord < nDim; lpCoord++){
		if (rngFile == NULL)
			rngNum = gsl_rng_uniform(rngGen);
		else
