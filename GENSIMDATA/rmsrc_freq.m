...
outDataDir = '~/Research/PulsarTiming/SimDATA/MultiSource/Investigation/Test11/BANDEDGE/2bands/RMSubsets/Int200';
mkdir(outDataDir);
...
binomega = omega(Index);
[~,Imax] = max(binsrcsnr);
OmgMax = omega(Imax);
Np = simParams.Np;
N = simParams.N;
Ilow = find(OmgMax - 5 <= binomega & binomega < OmgMax);
Iup = find(binomega > OmgMax & binomega <= OmgMax + 5); % find the sources within frequency interval [OmgMax-0.5, OmgMax + 0.5]
I = [Ilow Iup];
ite = length(I);
%I = 33; % debug
...
for j = 1:ite
    ...
    omega(Index(I(j))) = omega(Index(I(j))) - binomega(I(j));
end

newFile = strcat(outDataDir,filesep,FileName,'_rm_Freq_',num2str(OmgMax),ext);
