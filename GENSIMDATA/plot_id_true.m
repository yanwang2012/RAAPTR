...
idtyFilename = ['Identified_Src_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)];
matchedFilename = ['Matched_Sources_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)];
ext = '.mat';
...
for rlz = 1:Nrlzs
    ...
    % Use filtered true sources
    save([DataDir,filesep,simFileName,filesep,'simSrc_nm_sky_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)],'simRA_nm','simDec_nm','simSNR_nm');
    save([DataDir,filesep,simFileName,filesep,'matSrc_sky_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)],'matched_alpha_cnfrm','matched_dec_cnfrm','matched_snr_cnfrm', ...,
        'matched_freq_cnfrm','matched_alpha','matched_dec','matched_snr','matched_freq');
    save([DataDir,filesep,simFileName,filesep,'cnfrmSrc_sky_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)],'cnfrmRA','cnfrmDec','cnfrmSNR','cnfrmFreq');
    save([DataDir,filesep,simFileName,filesep,'idSrc_sky_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)],'idRA','idDec','idSNR','idFreq');
    ...
    saveas(gcf,[DataDir,filesep,'fig',filesep,simFileName,filesep,'SkyLocationC_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut),'.png'])
    savefig([DataDir,filesep,'fig',filesep,simFileName,filesep,'SkyLocationC_Est_SNR',num2str(SNR_threshold),'tSNR_',num2str(tSNR_cut)])
    close all
end
