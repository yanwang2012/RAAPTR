...
NsimFiles = dir([simDataDir,filesep,inputFileName]);
simFileNames = sort_nat({NsimFiles.name});
Nrlzs = length(NsimFiles);
spName = 'searchParams_Nyquist*';
FileList = dir([estDataDir,filesep,'*',inputFileName,'*.mat']); % get all estimated files

stage = 19; % iMBLT stages
for rlz = 1:Nrlzs
    [~,simFileBaseName,~] = fileparts(simFileNames{rlz});
    NspFiles = dir([searchParamsDir,filesep,simFileBaseName,spName]);
    bandNum = length(NspFiles); % number of band
    
    % select rlz
    filexp = ['\d+_',simFileBaseName,'(?=_|\.mat)_?\d{0,2}.mat'];
    ...
    for band = 1:bandNum
        if bandNum > 1
            outputfiles = dir([estDataDir,filesep,num2str(band),'_',simFileNames{rlz},'*.mat']);
        else
            outputfiles = dir([estDataDir,filesep,'*',simFileNames{rlz},'*.mat']);
        end
