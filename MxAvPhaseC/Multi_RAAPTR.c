...
int main(int argc, char *argv[])
{
	...
	if (argc != 7)
	{
		fprintf(stdout, "Usage: %s parameter_file_path input_file_path output_file_path mp_av_select psr_catalog number_of_iterations\n", argv[0]);
		return 1;
	}
	...
	for (int ite = 0; ite < num_ite; ite++)
	{
		...
		perfeval_omp_RAAPTR(ffp, inputFileName, outputFileName, mp_av_select, psr_catalog);
		...
		for (i = 0; i < Np; i++)
		{
			...
			for (j = 0; j < N[i]; j++)
			{
				...
				fprintf(fsub,"%.12f ", timResiduals[i][j]);
				fprintf(fest, "%.12f ", estRes[i][j]);
			}
			fprintf(fsub,"\n");
			fprintf(fest, "\n");
		}
	...
	*/
		...
		for(i = 0; i < Np; i++){
			...
			H5Ocopy(inFile, psrnames[i], ninFile, psrnames[i], H5P_DEFAULT, H5P_DEFAULT);
		...
		for (i = 0; i < Np; i++)
		{
			...
			hid_t dset_id = H5Dopen1(group_id, "timingResiduals"); // Open an existing dataset.
			status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, timResiduals[i]);
			// Close dataset.
			H5Dclose(dset_id);
			H5Gclose(group_id);
		}
