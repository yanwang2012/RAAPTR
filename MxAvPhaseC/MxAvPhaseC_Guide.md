...
# Main steps
The input data to be analyzed must be provided in the form of HDF5 files. You can see the format in the sample file TESTPERFEVALOMP/noise1.hdf5. Matlab codes have been provided in RAAPTR/GENSIMDATA to generate simulated data. A .mat file generated by these codes can be converted to the required HDF5 file using the Matlab code 'mpavinfile2hdf5.m' in RAAPTR/MxAvPhaseC. The output of the analysis is also stored in HDF5 files. For processing the results in Matlab, the code RAAPTR/MxAvPhaseC/mpavoutfile2mat.m can be used to convert the output HDF5 file to .mat file.

Besides the input data file, an HDF5 file containing the search parameters must be provided. Again, codes have been provided in RAAPTR/GENSIMDATA to generate a search parameter file and RAAPTR/MxAvPhaseC. 
